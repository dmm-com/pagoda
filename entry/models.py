    ...
    def get_value(self, with_metainfo=False, serialize=False):
        ...
        if (
            ...
            if self.parent_attr.schema.type & AttrTypeValue["named"]:
                ...
                value = [
                    _get_object_value(x) for x in self.data_array.all() if x.referral
                ]

            elif self.parent_attr.schema.type & AttrTypeValue["group"]:
                value = [
                    x for x in [_get_group_value(y) for y in self.data_array.all()] if x
                ]
...
class Attribute(ACLBase):
    ...
    def is_updated(self, recv_value):
        ...
        if self.schema.type == AttrTypeStr or self.schema.type == AttrTypeText:
            ...
            return last_value.value != AttributeValue.uniform_storable_for_group(
                recv_value
            )
            ...
            return sorted(
                [AttributeValue.uniform_storable_for_group(v) for v in recv_value if v]
            ) != sorted(
                [
                    x.value
                    for x in last_value.data_array.all()
        ...
        if (
            ...
            if self.schema.type & AttrTypeValue["named"]:
                ...
                return [
                    x
                    for x in [
                        AttributeValue.uniform_storable_for_group(y) for y in value
                    ]
                    if x
                ]
        ...
        if self.schema.type & AttrTypeValue["array"]:
            ...
            if self.schema.type & AttrTypeValue["named"]:
                ...
                updated_data = [
                    ...
                    for x in attrv.data_array.all()
                    if (
                        x.value != AttributeValue.uniform_storable_for_group(value)
                        and Group.objects.filter(id=x.value, is_active=True).exists()
                    )
                ]
...
class Entry(ACLBase):
    ...
    def may_remove_duplicate_attr(self, attr):
        ...
        """
        if self.attrs.filter(
            Q(schema=attr.schema, is_active=True), ~Q(id=attr.id)
        ).exists():
    ...
    def search_entries(
        ...
        for hint_entity_id in hint_entity_ids:
            ...
            search_result = make_search_results(
                user, resp, hint_attrs, limit, hint_referral
            )
