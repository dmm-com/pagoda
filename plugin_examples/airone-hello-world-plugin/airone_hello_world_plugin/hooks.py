...
def after_entry_create(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        logger.info(
            ...
            f"(ID: {getattr(instance, 'id', None)}) from model {sender.__name__ if sender else 'unknown'}"
        )
...
def custom_processing_hook(data, context=None):
    ...
    # Basic logging (in production, plugins should use proper interface implementations)
    logger.info(
        f"Hello World Plugin: Custom processing - data_type: {type(data).__name__}, context: {context}"
    )
