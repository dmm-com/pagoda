...
generr=$(docker compose --profile airone exec backend python manage.py spectacular 2>&1 > /dev/null | grep ERROR);
if [ "$generr" != "" ]; then
...
docker compose --profile airone exec backend python manage.py spectacular --file spec.yaml;
...
docker compose --profile airone exec frontend npx prettier --write "$dst"
