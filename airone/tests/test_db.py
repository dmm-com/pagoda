...
class ViewTest(AironeViewTest):
    ...
    def test_replicated_middleware(self):
        ...
        with self.assertLogs("django_replicated.router", level="DEBUG") as log:
            ...
            self.assertIn(
                "DEBUG:django_replicated.router:db_for_write: default", log.output
            )
        ...
        with self.assertLogs("django_replicated.router", level="DEBUG") as log:
            self.assertTrue(
                ...
                < (
                    time.time()
                    + django.conf.settings.REPLICATED_FORCE_MASTER_COOKIE_MAX_AGE
                )
            )
            ...
            self.assertNotIn(COOKIE_NAME, self.client.cookies)
            self.assertIn(
                "DEBUG:django_replicated.router:db_for_read: default", log.output
            )
