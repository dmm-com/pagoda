...
class SignalTest(TestCase):
    ...
    def test_create_aclbase_object(self):
        for i, model in enumerate([ACLBase, Entity, EntityAttr, Entry, Attribute]):
            ...
            self.assertTrue(
                Permission.objects.filter(name="readable", codename="%s.2" % obj.id)
            )
            self.assertTrue(
                Permission.objects.filter(name="writable", codename="%s.4" % obj.id)
            )
            self.assertTrue(
                Permission.objects.filter(name="full", codename="%s.8" % obj.id)
            )
    ...
    def test_edit_aclebase_object(self):
        for i, model in enumerate([ACLBase, Entity, EntityAttr, Entry, Attribute]):
            ...
            self.assertTrue(
                model.objects.filter(name=str(i) + "_new", is_active=True).exists()
            )
